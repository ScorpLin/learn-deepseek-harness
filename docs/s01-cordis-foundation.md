...
**名字的由来**：`cordis` 是拉丁语「心脏」（`cor` 的属格）。一个插件框架叫「心脏」，正好对上它的设计：心脏只**泵血**（总线让事件、服务在插件间流动），不决定你是什么（框架薄、插件厚）；器官**装得上也摘得下**（可逆的挂载/卸载，effect 负责清理）；心脏**持续跳动**维持生命（生命周期持续追踪）。它不叫「底座」「骨架」，偏叫「心脏」——因为它是活的中枢，不是死的基座。
